...
export function AdaptiveSuggestions() {
  const { userProgress, generateQuestSuggestions } = useGamification();
  const suggestions = generateQuestSuggestions();

  const getWeakestAreas = () => {
    return Object.entries(userProgress.areaStats)
      ...
      <CardContent className="space-y-6">
        ...
        {weakestAreas.length > 0 && (
          <div>
            ...
            <div className="space-y-3">
              {weakestAreas.map(({ area, accuracy, total }) => (
          ...
          <div>
            ...
            <div className="space-y-3">
              {suggestions.map(quest => (
                <div key={quest.id} className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                  <div className="flex items-center justify-between mb-2">
        ...
        <div className="p-4 bg-gradient-to-r from-orange-50 to-yellow-50 dark:from-orange-900/20 dark:to-yellow-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-medium text-orange-800 dark:text-orange-200">
              {userProgress.streakDias > 0 
                ? `Mantenha seu streak de ${userProgress.streakDias} dias!`
          ...
          <p className="text-sm text-orange-700 dark:text-orange-300">
            {userProgress.streakDias > 0
              ? `Continue estudando para ganhar +${Math.min(userProgress.streakDias * 2 + 10, 100)} XP amanhã`
              : 'Responda pelo menos 1 questão para iniciar seu streak'
            }
          </p>
        </div>
      </CardContent>
