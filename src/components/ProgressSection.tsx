...
export function ProgressSection() {
  ...
  return <Button variant="ghost" className="h-7 md:h-8 lg:h-9 bg-gradient-to-r from-blue-50 to-purple-50 hover:from-blue-100 hover:to-purple-100 border border-blue-200/50 rounded-md md:rounded-lg transition-all duration-300 shadow-sm hover:shadow-md group min-w-0 flex-shrink-0 text-left md:px-2 lg:px-3 text-gray-500 text-base my-0 py-0 px-[25px]">
      <div className="flex items-center gap-0.5 md:gap-1 lg:gap-1.5 min-w-0">
        {/* Ícone e Nível de Gamificação */}
        <div className="flex items-center gap-0.5 flex-shrink-0">
          <div className="w-3 h-3 md:w-4 md:h-4 lg:w-5 lg:h-5 bg-gradient-to-r from-orange-400 to-orange-600 rounded-full flex items-center justify-center px-[11px] mx-[8px]">
            <Zap className="w-1.5 h-1.5 md:w-2 md:h-2 lg:w-2.5 lg:h-2.5 text-white" />
          </div>
          <span className="md:text-sm font-semibold text-gray-800 text-base">
        ...
        <div className="flex items-center gap-0.5 md:gap-1 min-w-0">
          <span className="font-medium text-gray-600 whitespace-nowrap text-base">
            {userProgress.xp}/{userProgress.xpToNextLevel}
          </span>
          
          {/* Barra de Progresso XP - mais estreita */}
          <div className="relative w-4 md:w-6 lg:w-8 h-1 md:h-1.5 bg-gray-200 rounded-full overflow-hidden flex-shrink-0 px-[36px]">
